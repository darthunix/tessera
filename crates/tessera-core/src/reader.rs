...
pub trait ColumnReader {
    ...
    /// Different row counts fail immediately. Empty words are skipped; each
    /// nonempty word gets the same readiness validation as [`Self::word_values`]
    /// before reading it. The default implementation delegates to that method.
    /// The first failing word yields one error and ends iteration permanently.
    /// Rows already yielded are not rolled back. This operation mutates no data.
    ///
    /// Iteration can resume after partial consumption with `next()` or
    /// [`Iterator::try_fold`]. Use `try_fold` to stop on a consumer error:
    /// [`Iterator::fold`] does not inspect the accumulator for errors and
    /// continues calling the consumer. Both stop at the first reader error.
    fn selected_values<'a>(
        &'a self,
        rows: &'a RowMaskView<'_>,
    ) -> Result<impl Iterator<Item = Result<(usize, Option<Self::Value>)>> + 'a> {
        ensure!(
            self.nrows() == rows.nrows(),
            "column and selection row counts differ"
        );
        Ok(SelectedValues {
            rows: *rows,
            next_word: 0,
            end_word: word_count(rows.nrows()),
            current: None,
            make_word: move |index, selected| self.word_values(index, selected),
        })
    }
}

struct SelectedValues<'a, F, I> {
    rows: RowMaskView<'a>,
    next_word: usize,
    end_word: usize,
    current: Option<I>,
...
impl<F, I, V> Iterator for SelectedValues<'_, F, I>
where
    F: FnMut(usize, u64) -> Result<I>,
    I: Iterator<Item = (usize, Option<V>)>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            match (self.make_word)(index, selected) {
        ...
        G: FnMut(B, Self::Item) -> B,
    {
        if let Some(current) = self.current.take() {
            acc = current.fold(acc, |acc, value| fold(acc, Ok(value)));
        }
        while self.next_word < self.end_word {
            ...
            match (self.make_word)(index, selected) {
                Ok(current) => acc = current.fold(acc, |acc, value| fold(acc, Ok(value))),
